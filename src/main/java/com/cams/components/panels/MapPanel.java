    ...
    public MapPanel(){
            ...
            public void mouseClicked(MouseEvent e){
                ...
                if (e.getButton() == MouseEvent.BUTTON1) { 
                    if(Util.Current!=null){
                        ...
                        JOptionPane.showMessageDialog(Util.Main,"Please set simulation environment first!.","Empty simulation environment",JOptionPane.WARNING_MESSAGE); 
                    }
                }
        ...
        this.addMouseMotionListener(new MouseAdapter(){
            ...
            public void mouseMoved(MouseEvent e){
                ...
                if(Util.tooltipLabel.isVisible())Util.tooltipLabel.setLocation(e.getX() + 10, e.getY() + 10);
                repaint();
            }
        ...
        if(Util.Current!=null){
            switch (Util.Toolstate) {
                ...
                case 2:
                    ...
                    if(runwayWidth==0)runwayWidth=6;
                ...
                case 3:
                    ...
                    if(route!=null){
                        for(int i=0;i<route.routelength()-1;i++){
                            ...
                            g2d = (Graphics2D) g.create();
                            Line2D.Double line = new Line2D.Double((-offX+(int)Util.Current.Waypoints[idw1].getX())*DrawingFactor, (-offY+(int)Util.Current.Waypoints[idw1].getY())*DrawingFactor, (-offX+(int)Util.Current.Waypoints[idw2].getX())*DrawingFactor, (-offY+(int)Util.Current.Waypoints[idw2].getY())*DrawingFactor);
                            g2d.draw(line);
                        }
                        if(idw2==-1)idw2=route.getWaypoints()[0];
                        if(Util.Current.Waypoints[idw2]!=null){
                            g2d = (Graphics2D) g.create();
                            Line2D.Double line =new Line2D.Double((-offX+(int)Util.Current.Waypoints[idw2].getX())*DrawingFactor, (-offY+(int)Util.Current.Waypoints[idw2].getY())*DrawingFactor, Util.MousePosX,Util.MousePosY);
                            g2d.draw(line);
                        }
                    }
                    break;
            ...
            for (Waypoint waypoint : Util.Current.Waypoints) {
                ...
                if (waypoint != null){
                    if(waypoint.getType()==0){
                        Graphics2D g2d = (Graphics2D) g.create();
                        g2d.translate((-offX+(double) waypoint.getX())*DrawingFactor, (-offY+(double) waypoint.getY())*DrawingFactor);
                        g2d.fillOval( -5,  -5, 10, 10);
            ...
            for (Runway runway : Util.Current.Runways) {
                ...
                if (runway != null) {
                    ...
                    double angle = runway.getAngle(); // 获取跑道的角度，具体方法根据你的实际情况调整

                    // 创建 Graphics2D 对象
                    Graphics2D g2d = (Graphics2D) g.create();
                    g2d.translate((-offX+(int) runway.getX())*DrawingFactor, (-offY+(int) runway.getY())*DrawingFactor);
